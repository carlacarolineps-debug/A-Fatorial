...
/* A porcentagem escrita por extenso nas pontas.

   Arredondar sozinho mente nos dois extremos: uma resposta em 326 vira
   "0%", e 325 em 326 vira "100%", que diz que ninguem escolheu a outra e
   que todas escolheram esta. Nas pontas a tela escreve a palavra. */
function formPcTexto(quantas, total) {
  const q = Number(quantas || 0);
  const t = Number(total || 0);
  const pc = formPorCento(q, t);
  if (q > 0 && pc === 0) return 'menos de 1%';
  if (q < t && pc === 100) return 'quase 100%';
  return pc + '%';
}

/* A barra da perda: a parte cheia e quem seguiu, a marcada e quem parou.

   Uma barra so, com as duas partes na mesma linha, diz de relance o que
   duas barras separadas obrigariam a comparar de olho. */
function formBarraPerda(viu, parou) {
  const v = Number(viu || 0);
  const p = Math.max(0, Math.min(v, Number(parou || 0)));
  if (!v) return '<div class="barra-perda"><i class="vazia"></i></div>';
  const pctPerda = (p / v) * 100;
  return '<div class="barra-perda" title="' + (v - p) + ' seguiram, ' + p + ' pararam">' +
    '<i class="seguiu" style="width:' + (100 - pctPerda).toFixed(1) + '%"></i>' +
    '<i class="perdeu" style="width:' + pctPerda.toFixed(1) + '%"></i>' +
  '</div>';
}
...
function formMedCorpo() {
  ...
  // 3b. O que os números estão dizendo.
  html += formMedInsightsHtml(perguntas, poucos);

  // 4. A escada do abandono, pergunta a pergunta.
  //
  // Antes a barra media quantos CHEGARAM em cada pergunta, e a taxa de
  // abandono so aparecia nas duas piores. Quem abre a tela quer saber
  // exatamente onde perde gente, e isso e um numero em toda linha, nao um
  // comprimento de barra para comparar de olho. Agora cada pergunta mostra
  // a barra dividida: a parte cheia e quem seguiu, a parte marcada e quem
  // parou ali, e a taxa vem escrita ao lado.
  if (perguntas.length) {
    html += '<div class="cartao">' +
      '<div class="cartao-t">Onde as pessoas param' +
        '<span class="graf-legenda">' +
          '<span class="graf-chave"><i class="bloco"></i>seguiu</span>' +
          '<span class="graf-chave"><i class="bloco perdeu"></i>parou aqui</span>' +
        '</span>' +
      '</div>' +
      '<p class="dica" style="margin:-8px 0 18px">De cada 100 que chegam na pergunta, quantas param ' +
      'ali sem responder. A pergunta que mais derruba é a primeira coisa a mexer.</p>' +
      '<div class="rolo-h"><table class="lista"><thead><tr>' +
        '<th>Pergunta</th><th class="num">Chegaram</th><th class="num">Responderam</th>' +
        '<th class="num">Pararam</th><th style="width:34%">Quanto perde</th>' +
      '</tr></thead><tbody>' +
      perguntas.map(function (p, i) {
        const viu = Number(p.viu || 0);
        const parou = Number(p.abandonou || 0);
        const pc = formPorCento(parou, viu);
        const alto = !poucos && pc > FORM_ABANDONO_ALTO;
        return '<tr>' +
          '<td data-t><b' + (p.ativa === false ? ' style="opacity:.65"' : '') + '>' +
            (i + 1) + '. ' + esc(p.titulo || p.chave) + '</b>' +
            (p.ativa === false ? ' <span class="eti eti-neutra">saiu do formulário</span>' : '') + '</td>' +
          '<td class="num" data-r="chegaram">' + viu + '</td>' +
          '<td class="num" data-r="responderam">' + Number(p.respondeu || 0) + '</td>' +
          '<td class="num" data-r="pararam">' +
            (parou
      ...
      (m.pior_pergunta && !poucos
        ? '<p style="font-size:14px;color:var(--tx-2);margin-top:18px">A que mais derruba é ' +
          '<b>"' + esc(formTituloDaChave(m.pior_pergunta.chave, perguntas)) + '"</b>: de cada 100 que ' +
    ...
    // 6. As respostas ate agora, uma pergunta por bloco.
    //
    // Antes era uma lista corrida de TODA opcao de TODA pergunta, na ordem
    // em que foram escritas, com as que ninguem marcou ocupando o mesmo
    // espaco das outras: vinte e cinco linhas em que vinte diziam "0
    // pessoas, 0 de cada 100". Quem olha isso quer saber duas coisas, e as
    // duas ficavam escondidas: qual opcao ganhou, e o quanto ela ganhou.
    //
    // Agora cada pergunta e um bloco: a mais marcada em cima, escrita por
    // extenso, as outras em ordem, e as que ninguem marcou recolhidas numa
    // linha so, que continua dizendo quais sao.
    ...
    if (comOpcoes.length) {
      html += '<div class="cartao">' +
        ...
        'marcou. Em cada pergunta, a mais escolhida vem primeiro.</p>' +
        '<div class="respostas">' +
        comOpcoes.map(function (p) {
          const total = p.opcoes.reduce(function (t, o) { return t + Number(o.quantas || 0); }, 0);
          const ordenadas = p.opcoes.slice().sort(function (a, b) {
            return Number(b.quantas || 0) - Number(a.quantas || 0);
          });
          const marcadas = ordenadas.filter(function (o) { return Number(o.quantas || 0) > 0; });
          const zeradas = ordenadas.filter(function (o) { return !Number(o.quantas || 0); });
          const lider = marcadas[0];

          return '<div class="resposta-bloco">' +
            '<div class="resposta-t">' +
              '<b>' + esc(p.titulo || p.chave) + '</b>' +
              '<span class="dica">' + total + (total === 1 ? ' resposta' : ' respostas') + '</span>' +
            '</div>' +

            (!total
              ? '<p class="dica">Ninguém respondeu esta pergunta ainda.</p>'
              /* A frase so nasce quando ha o que comparar. Com uma opcao
                 marcada so, ela repetia a unica linha da lista logo abaixo,
                 e "100 de cada 100 escolheram" e frase que ninguem escreve. */
              : (lider && !poucos && marcadas.length > 1
                  ? '<p class="resposta-lider">' +
                      (formPorCento(lider.quantas, total) >= 99
                        ? '<b>Quase todas</b> escolheram '
                        : '<b>' + formPorCento(lider.quantas, total) + ' de cada 100</b> escolheram ') +
                      '"' + esc(lider.texto) + '".</p>'
                  : '') +
                marcadas.map(function (o) {
                  const quantas = Number(o.quantas || 0);
                  const pc = formPorCento(quantas, total);
                  return '<div class="resposta-linha' + (o === lider ? ' lider' : '') + '">' +
                    '<span class="resposta-nome">' + esc(o.texto) + '</span>' +
                    /* Uma resposta em 326 arredonda para 0%, e "1 · 0%" na
                       mesma linha se le como erro de conta. Abaixo de 1 a
                       tela escreve que e menos de 1, e nao zero. */
                    '<span class="resposta-conta">' + quantas +
                      (!poucos ? ' · ' + formPcTexto(quantas, total) : '') + '</span>' +
                    formBarra(pc, o === lider ? 'var(--o)' : 'var(--o-35)') +
                  '</div>';
                }).join('') +
                (zeradas.length
                  ? '<p class="dica" style="margin-top:10px">' +
                      (zeradas.length === 1
                        ? 'Ninguém escolheu '
                        : 'Ninguém escolheu ' + (marcadas.length ? 'as outras ' : 'as ') +
                          zeradas.length + ': ') +
                      esc(zeradas.map(function (o) { return o.texto; }).join(', ')) + '.</p>'
                  : '')) +
          '</div>';
        }).join('') +
        '</div></div>';
...
/* ---------------------------------------------------------------------
   ...
   O resto da tela mostra o dado; este bloco lê o dado e diz o que fazer
   com ele. É a diferença entre um relatório e um painel: sozinha, a
   pessoa que abre isso uma vez por semana tem que comparar sete blocos de
   cabeça para chegar em "a pergunta 5 está derrubando um quarto da fila".

   Cada linha só nasce quando é verdade E quando passa de um corte que
   valha a pena mexer. Nada aqui aparece sempre: bloco que aparece sempre
   deixa de ser lido na segunda semana, que é a regra da casa.
   --------------------------------------------------------------------- */

const FORM_INSIGHT_MIN = 20;   // abaixo disso, dois envios mudam qualquer conta

function formMedInsightsHtml(perguntas, poucos) {
  const m = FORM.med || {};
  const f = m.funil || {};
  const abriu = Number(f.abriu || 0);
  if (poucos || abriu < FORM_INSIGHT_MIN) return '';

  const achados = [];
  const somar = function (tom, icone, titulo, texto, acao) {
    achados.push({ tom: tom, ic: icone, t: titulo, x: texto, acao: acao || '' });
  };

  // 1. A pergunta que mais derruba.
  const comViu = perguntas.filter(function (p) { return Number(p.viu || 0) >= 10; });
  const pior = comViu.slice().sort(function (a, b) {
    return formPorCento(b.abandonou, b.viu) - formPorCento(a.abandonou, a.viu);
  })[0];
  if (pior && formPorCento(pior.abandonou, pior.viu) > FORM_ABANDONO_ALTO) {
    const pc = formPorCento(pior.abandonou, pior.viu);
    const custo = Number(pior.mediana_ms || 0);
    somar('alerta', 'i-target', 'A pergunta que mais derruba é "' + (pior.titulo || pior.chave) + '".',
      'De cada 100 que chegam nela, ' + pc + ' param ali' +
      (custo ? ', e ela leva ' + formTempo(custo) + ' para responder' : '') + '. ' +
      'Encurtar o texto, virar opcional ou mandar para o fim são as três saídas.',
      '<button class="bt bt-linha bt-sm" onclick="formVerPergunta(\'' + esc(pior.chave) + '\')">Ver essa pergunta</button>');
  }

  // 2. A pergunta que custa caro perto das outras.
  const comTempo = perguntas.filter(function (p) { return Number(p.mediana_ms || 0) > 0; });
  if (comTempo.length > 2) {
    const cara = comTempo.slice().sort(function (a, b) { return b.mediana_ms - a.mediana_ms; })[0];
    const resto = comTempo.filter(function (p) { return p !== cara; });
    const somaResto = resto.reduce(function (t, p) { return t + Number(p.mediana_ms); }, 0);
    if (Number(cara.mediana_ms) > somaResto * 0.5) {
      somar('atencao', 'i-clock', '"' + (cara.titulo || cara.chave) + '" leva ' + formTempo(cara.mediana_ms) + '.',
        'As outras ' + resto.length + ' juntas levam ' + formTempo(somaResto) + '. Uma pergunta sozinha ' +
        'custando quase o que todas as outras custam é a que mais pesa na desistência de quem tem pressa.');
    }
  }

  // 3. Onde a conversao muda: aparelho.
  const ap = (m.por_aparelho || []).filter(function (a) { return Number(a.visitas || 0) >= 15; });
  if (ap.length > 1) {
    const ordenado = ap.slice().sort(function (a, b) {
      return formPorCento(b.enviou, b.visitas) - formPorCento(a.enviou, a.visitas);
    });
    const melhor = ordenado[0], piorAp = ordenado[ordenado.length - 1];
    const dif = formPorCento(melhor.enviou, melhor.visitas) - formPorCento(piorAp.enviou, piorAp.visitas);
    if (dif >= 12) {
      somar('atencao', 'i-grid', 'No ' + esc(piorAp.aparelho) + ' termina bem menos gente.',
        'No ' + esc(melhor.aparelho) + ', ' + formPorCento(melhor.enviou, melhor.visitas) +
        ' de cada 100 terminam; no ' + esc(piorAp.aparelho) + ', ' +
        formPorCento(piorAp.enviou, piorAp.visitas) + '. Diferença dessa altura costuma ser tamanho de ' +
        'letra, de botão ou campo que o teclado cobre.');
    }
  }

  // 4. Onde a conversao muda: origem.
  const og = (m.por_origem || []).filter(function (o) { return Number(o.visitas || 0) >= 15; });
  if (og.length > 1) {
    const ordenado = og.slice().sort(function (a, b) {
      return formPorCento(b.enviou, b.visitas) - formPorCento(a.enviou, a.visitas);
    });
    const melhor = ordenado[0], piorOg = ordenado[ordenado.length - 1];
    const difOg = formPorCento(melhor.enviou, melhor.visitas) - formPorCento(piorOg.enviou, piorOg.visitas);
    if (difOg >= 15) {
      somar('info', 'i-compass', 'Quem vem de ' + esc(melhor.origem) + ' termina mais.',
        formPorCento(melhor.enviou, melhor.visitas) + ' de cada 100 de ' + esc(melhor.origem) +
        ' terminam, contra ' + formPorCento(piorOg.enviou, piorOg.visitas) + ' de ' + esc(piorOg.origem) +
        '. Vale saber para onde mandar o esforço.');
    }
  }

  // 5. Quem abre e nem comeca.
  const comecou = Number(f.comecou || 0);
  const pcComeca = formPorCento(comecou, abriu);
  if (abriu >= 30 && pcComeca < 70) {
    somar('atencao', 'i-flag', (100 - pcComeca) + ' de cada 100 abrem e nem começam.',
      'Elas não passam da capa. Isso é texto de capa ou promessa que não bateu com o que o botão da ' +
      'landing prometeu, e não é o formulário: quem desiste ali não viu pergunta nenhuma.');
  }

  // 6. O periodo comparado com o anterior.
  const fa = (FORM.medAntes && FORM.medAntes.funil) || null;
  if (fa && Number(fa.enviou || 0) >= 10) {
    const antes = Number(fa.enviou), agora = Number(f.enviou || 0);
    const pct = Math.round(Math.abs(agora - antes) / antes * 100);
    if (pct >= 20) {
      somar(agora > antes ? 'ok' : 'alerta', 'i-trend',
        (agora > antes ? 'As aplicações subiram ' : 'As aplicações caíram ') + pct + '%.',
        'Foram ' + agora + ' neste período, contra ' + antes + ' no anterior, do mesmo tamanho.' +
        (agora < antes ? ' Vale olhar se alguma origem parou de trazer gente.' : ''));
    }
  }

  // 7. Campo que recusa o que a pessoa escreve.
  const comErro = perguntas.filter(function (p) { return Number(p.erro_campo || 0) > 0 && Number(p.viu || 0) >= 15; })
    .sort(function (a, b) { return formPorCento(b.erro_campo, b.viu) - formPorCento(a.erro_campo, a.viu); })[0];
  if (comErro && formPorCento(comErro.erro_campo, comErro.viu) >= 10) {
    somar('atencao', 'i-x', 'O campo de "' + (comErro.titulo || comErro.chave) + '" recusa muita gente.',
      formPorCento(comErro.erro_campo, comErro.viu) + ' de cada 100 que chegam nele têm a resposta ' +
      'recusada antes de conseguir passar. Ou a regra está apertada demais, ou o campo não explica o ' +
      'que espera.');
  }

  // 8. Quando esta tudo bem, isso tambem e informacao, mas em UMA linha.
  if (!achados.length) {
    const dez = formCadaDez(f.enviou, f.comecou);
    return '<div class="cartao">' +
      '<div class="cartao-t">O que os números estão dizendo</div>' +
      '<p style="font-size:14px;color:var(--tx-2)">Nada fora do lugar neste período. ' +
      (dez !== null ? 'De cada 10 que começam, ' + dez + ' terminam, e nenhuma pergunta está derrubando ' +
        'mais que ' + FORM_ABANDONO_ALTO + ' de cada 100.' : '') + '</p>' +
    '</div>';
  }

  const ordem = { alerta: 0, atencao: 1, info: 2, ok: 3 };
  achados.sort(function (a, b) { return ordem[a.tom] - ordem[b.tom]; });

  return '<div class="cartao">' +
    '<div class="cartao-t">O que os números estão dizendo' +
      '<span class="conta-t">' + achados.length + (achados.length === 1 ? ' achado' : ' achados') + '</span>' +
    '</div>' +
    '<div class="insights">' +
      achados.map(function (a) {
        return '<div class="insight tom-' + a.tom + '">' +
          '<div class="insight-ic">' + ic(a.ic) + '</div>' +
...
/* =====================================================================
   ...
   O que estava errado, e nao era so feio:

   a. O eixo mentia. Quatro linhas de grade a topo/3 de distancia, com o
      numero de cada uma arredondado: com teto 4, o eixo escrevia 4, 3, 1
      e 0 em linhas igualmente espacadas. O 3 estava desenhado onde mora o
      2,67 e o 1 onde mora o 1,33. Quem lesse a altura de um ponto pelo
      eixo leria errado.

   b. A conta da escala estava escrita duas vezes, aqui e no balao que
      segue o mouse. Duas copias da mesma regra e uma promessa de que um
      dia o ponto do balao vai parar num lugar e a linha em outro.

   c. Com nenhum envio no periodo, a tela dizia "o dia mais cheio foi
      05/08, com 0" e desenhava o fio do pico apontando para o zero.

   O que ela ganhou junto: sabado e domingo pintados por tras, que e o que
   explica metade dos vales de um grafico de dias; area embaixo das duas
   linhas, e nao so da de baixo; e um ponto por dia quando o periodo cabe
   num mes, para uma linha rente ao zero se ler como "foi medido e deu
   zero" em vez de "isto esta quebrado".
   ===================================================================== */

/* A escala do eixo: teto redondo e degraus INTEIROS e iguais.

   Devolve o teto, o tamanho do degrau e quantos degraus. A regra e a de
   sempre: o degrau e 1, 2 ou 5 vezes uma potencia de dez, e o eixo tem
   entre dois e cinco degraus. Abaixo de seis, o degrau e 1 e o eixo conta
   de um em um, porque nao existe meia aplicacao. */
function formEscala(maiorValor) {
  const bruto = Math.max(1, Math.ceil(Number(maiorValor) || 0));
  if (bruto <= 5) {
    const n = Math.max(2, bruto);
    return { topo: n, passo: 1, divisoes: n };
  }
  for (let mag = 0; mag < 9; mag++) {
    const dez = Math.pow(10, mag);
    const candidatos = [1 * dez, 2 * dez, 5 * dez];
    for (let k = 0; k < candidatos.length; k++) {
      const passo = candidatos[k];
      const n = Math.ceil(bruto / passo);
      if (n >= 2 && n <= 5) return { topo: passo * n, passo: passo, divisoes: n };
    }
  }
  return { topo: bruto, passo: bruto, divisoes: 1 };
}

// Um lugar so para a escala do "Dia a dia": o desenho e o balao leem
// daqui, e por isso nao tem como discordarem.
function formEscalaDoDia(dias) {
  return formEscala(Math.max.apply(null,
    dias.map(function (d) { return Number(d.abriu || 0); }).concat([1])));
}

// Sabado e domingo, para pintar por tras. Sem fuso: a data vem do
// servidor como AAAA-MM-DD, e ler isso com new Date() joga o dia para
// tras em quem esta a oeste de Londres.
function formFimDeSemana(dia) {
...
function formMedDiaHtml() {
  ...
  const alturaUtil = g.alt - g.t - g.b;
  const larguraUtil = g.larg - g.l - g.r;
  const esc_ = formEscalaDoDia(dias);
  const topo = esc_.topo;

  const abriu = formCaminho(dias, 'abriu', topo, false);
  const abriuArea = formCaminho(dias, 'abriu', topo, true);
  ...
  const temPico = Number(maior.enviou || 0) > 0;

  // As linhas da grade e os numeros do eixo, agora nos mesmos lugares.
  let grade = '';
  let escada = '';
  for (let i = 0; i <= esc_.divisoes; i++) {
    const y = g.t + (alturaUtil / esc_.divisoes) * i;
    const valor = topo - esc_.passo * i;
    grade += '<line x1="0" y1="' + y.toFixed(1) + '" x2="' + g.larg +
      '" y2="' + y.toFixed(1) + '" stroke="var(--fio-2)"' +
      (i === esc_.divisoes ? '' : ' stroke-dasharray="3 6"') + '/>';
    escada += '<span style="top:' + ((y / g.alt) * 100).toFixed(2) + '%">' + valor + '</span>';
  }

  // Sabado e domingo por tras de tudo. Metade dos vales de um grafico de
  // dias e fim de semana, e sem isso a pessoa procura explicacao para uma
  // queda que so quer dizer que era domingo.
  const passoX = larguraUtil / (dias.length - 1);
  let fimDeSemana = '';
  dias.forEach(function (d, i) {
    if (!formFimDeSemana(d.dia)) return;
    // A faixa vai de meio passo antes ate meio passo depois do ponto, e e
    // aparada nas bordas: no primeiro e no ultimo dia, metade dela cairia
    // fora do desenho e a faixa apareceria deslocada para dentro.
    const centro = g.l + i * passoX;
    const x0 = Math.max(0, centro - passoX / 2);
    const x1 = Math.min(g.larg, centro + passoX / 2);
    if (x1 <= x0) return;
    fimDeSemana += '<rect x="' + x0.toFixed(1) + '" y="' + g.t +
      '" width="' + (x1 - x0).toFixed(1) + '" height="' + alturaUtil +
      '" fill="var(--fio-2)" opacity=".55"/>';
  });

  /* Um ponto por dia quando o periodo cabe num mes. Numa linha rente ao
     zero, os pontos sao a diferenca entre "foi medido e deu zero" e "isto
     esta quebrado".

     Eles moram FORA do SVG, como os numeros do eixo e pela mesma razao: o
     desenho estica na horizontal para caber na largura da tela, e circulo
     dentro dele estica junto. No computador isso ja deixava os pontos
     levemente ovais; no telefone, onde 900 unidades entram em 250 pixels,
     eles viravam tracinhos deitados. Em HTML, posicionados em
     porcentagem, ficam redondos em qualquer largura. */
  const emPorCento = function (pt) {
    return 'left:' + ((pt[0] / g.larg) * 100).toFixed(3) + '%;' +
           'top:' + ((pt[1] / g.alt) * 100).toFixed(3) + '%';
  };
  let pontos = '';
  if (dias.length <= 31) {
    pontos = enviou.pontos.map(function (pt, i) {
      const v = Number(dias[i].enviou || 0);
      return '<i class="graf-ponto dia' + (v ? '' : ' zero') + '" style="' + emPorCento(pt) + '"></i>';
    }).join('');
  }
  // O pico so ganha fio e ponto quando existe pico. Apontar para o zero
  // era dizer que o dia mais cheio do mes teve nenhuma.
  const marcaDoPico = temPico
    ? '<i class="graf-ponto pico" style="' + emPorCento(enviou.pontos[iMaior]) + '"></i>'
    : '';
  ...
  const svg =
    '<svg viewBox="0 0 ' + g.larg + ' ' + g.alt + '" preserveAspectRatio="none" ' +
      ...
      '<defs>' +
        '<linearGradient id="grafArea" x1="0" y1="0" x2="0" y2="1">' +
          ...
          '<stop offset="100%" stop-color="var(--o)" stop-opacity="0"/>' +
        '</linearGradient>' +
        '<linearGradient id="grafAreaAbriu" x1="0" y1="0" x2="0" y2="1">' +
          '<stop offset="0%" stop-color="var(--tx-3)" stop-opacity=".16"/>' +
          '<stop offset="100%" stop-color="var(--tx-3)" stop-opacity="0"/>' +
        '</linearGradient>' +
        // A linha da casa desenhada duas vezes: a de baixo borrada, que e
        // o brilho, e a de cima nitida.
      ...
      fimDeSemana +
      grade +
      '<path d="' + abriuArea.d + '" fill="url(#grafAreaAbriu)"/>' +
      '<path d="' + enviouArea.d + '" fill="url(#grafArea)"/>' +
      '<path d="' + abriu.d + '" fill="none" stroke="var(--tx-3)" stroke-width="1.75" ' +
      ...
      '<path d="' + enviou.d + '" fill="none" stroke="var(--o)" stroke-width="2.5" ' +
        'stroke-linejoin="round" stroke-linecap="round" vector-effect="non-scaling-stroke"/>' +
      // O fio do pico, e o fio que segue o mouse. Linha vertical nao sofre
      // com o esticao horizontal: ela continua sendo uma linha vertical.
      (temPico
    ...
    '</svg>' +
    pontos + marcaDoPico +
    // Os dois pontos que o mouse acende, tambem em HTML.
    '<i class="graf-ponto abriu" id="form-graf-ponto-abriu" hidden></i>' +
    '<i class="graf-ponto agora" id="form-graf-ponto" hidden></i>';

  // As datas. Ate dez dias cabem todas, e ai cada ponto tem nome: numa
  // semana, "primeira, meio e ultima" obrigava a contar nos dedos para
  // saber que dia era o terceiro pico. Acima disso sao quatro, que e o
  // que cabe sem uma encostar na outra.
  let marcas;
  if (dias.length <= 10) {
    marcas = dias.map(function (d, i) { return i; });
  } else if (dias.length <= 20) {
    marcas = [0, Math.floor(dias.length / 2), dias.length - 1];
  } else {
    marcas = [0, Math.floor(dias.length / 3), Math.floor((dias.length * 2) / 3), dias.length - 1];
  }

  const totalEnviou = dias.reduce(function (t, d) { return t + Number(d.enviou || 0); }, 0);
  const totalAbriu = dias.reduce(function (t, d) { return t + Number(d.abriu || 0); }, 0);
  const frase = temPico
    ? 'O dia mais cheio foi ' + esc(dataCurta(maior.dia)) + ', com ' + Number(maior.enviou) +
      (Number(maior.enviou) === 1 ? ' aplicação.' : ' aplicações.') +
      ' Toque na linha, ou passe o mouse, para ver um dia. A conta de cada 10 só aparece no ' +
      'total do período: em um dia só, duas pessoas mudam tudo.'
    : (totalAbriu
        ? 'Ninguém terminou o formulário neste período. A linha cheia fica no zero e a tracejada ' +
          'mostra quem abriu: é entre as duas que está o que se perde. Toque na linha, ou passe o ' +
          'mouse, para ver um dia.'
        : 'Ninguém abriu o formulário neste período.');

  return '<div class="cartao">' +
    '<div class="cartao-t">Dia a dia' +
      '<span class="graf-legenda">' +
        '<span class="graf-chave"><i class="cheia"></i>terminaram' +
          (totalEnviou ? ' <b>' + totalEnviou + '</b>' : '') + '</span>' +
        '<span class="graf-chave"><i class="tracejada"></i>abriram' +
          (totalAbriu ? ' <b>' + totalAbriu + '</b>' : '') + '</span>' +
        '<span class="graf-chave"><i class="banda"></i>fim de semana</span>' +
      '</span>' +
    '</div>' +
    '<div class="graf-linha">' +
      '<div class="graf-eixo">' + escada + '</div>' +
      '<div class="graf-caixa' + (dias.length <= 10 ? ' poucos' : '') +
        '" id="form-graf-caixa">' + svg +
        '<div class="graf-balao" id="form-graf-balao" hidden></div>' +
      '</div>' +
    '</div>' +
    '<div class="graf-datas">' +
      marcas.map(function (i) { return '<span>' + esc(dataCurta(dias[i].dia)) + '</span>'; }).join('') +
    '</div>' +
    '<p class="dica" style="margin-top:10px">' + frase + '</p>' +
  '</div>';
}
...
/* O balão que segue o mouse.
   ...
   A escala vem de formEscalaDoDia, a mesma que desenhou as linhas. Antes
   ela estava copiada aqui, e uma cópia de regra é a promessa de que um
   dia o ponto do balão vai parar num lugar e a linha em outro. */
function formGrafLigar() {
  ...
  const pontoAbriu = porId('form-graf-ponto-abriu');
  ...
  if (dias.length < 2) return;
  const g = FORM_GRAF;
  const topo = formEscalaDoDia(dias).topo;
  const alturaUtil = g.alt - g.t - g.b;
  const y = function (v) { return g.t + alturaUtil - (Number(v || 0) / topo) * alturaUtil; };
  ...
  const mover = function (ev) {
    ...
    // Os pontos sao HTML, e por isso ficam redondos em qualquer largura:
    // a posicao deles e em porcentagem da caixa, igual a dos numeros do
    // eixo.
    const por = ((xSvg / g.larg) * 100).toFixed(3) + '%';
    if (ponto) {
      ponto.hidden = false;
      ponto.style.left = por;
      ponto.style.top = ((y(d.enviou) / g.alt) * 100).toFixed(3) + '%';
    }
    // O segundo ponto e o da linha tracejada. Sem ele, o balao dizia dois
    // numeros e o desenho so confirmava um.
    if (pontoAbriu) {
      pontoAbriu.hidden = false;
      pontoAbriu.style.left = por;
      pontoAbriu.style.top = ((y(d.abriu) / g.alt) * 100).toFixed(3) + '%';
    }

    balao.hidden = false;
    balao.innerHTML = '<b>' + esc(dataCurta(d.dia)) +
        (formFimDeSemana(d.dia) ? '<em>fim de semana</em>' : '') + '</b>' +
  ...
  const sair = function () {
    ...
    if (pontoAbriu) pontoAbriu.hidden = true;
  };
