...
| **AUTO CONTROLE em todas as etapas (arte + medidas + puxada)** | Além do Clichê, as etapas **Impressão · Laminação · Rebobinadeira · Corte & Solda · Especificação Final** ganharam o bloco **AUTO CONTROLE** das fichas oficiais: **medidas** (largura · comprimento/altura · sanfona · aba), a **imagem da arte anexada dentro do croqui com as réguas de cotagem em volta**, e o **sentido de puxada / desbobinamento** (os 4 desenhos oficiais de bobina). Cada etapa tem os seus próprios campos (`fu_<pref>_ac_*`), na cor da etapa, e tudo **sai na impressão** da instrução de trabalho daquele operador. Clique no croqui para ampliar. Funções: `autoCtrlHTML`, `fuAutoCtrlInit`, `fuAutoCtrlCroqui`, `croquiSVG` (gerador único de croqui). |
| **Croqui da arte com a imagem dentro e medidas em volta** | Preencha **Largura · Altura · Sanfona · Aba** e **anexe a arte**: a imagem entra **dentro do retângulo do croqui** e as **réguas de medição são desenhadas em volta dela** (largura embaixo, altura à esquerda, sanfona/aba tracejadas sobre a imagem) — como na ficha original, num quadro único (não em quadros separados). A arte é salva com a ficha e **sai na impressão** dentro do croqui (SVG `<image>`), sem precisar ir à Documentação Visual. Clique no croqui para **ampliar em tela cheia**. Funções: `fuCliCroqui` (embute a arte + cotas), `fuCliArteUpload`, `fuCliArteZoom`. |
| **Registro de Devolução (RGQ-45)** | Ficha de Devolução alinhada ao modelo oficial **RGQ-45**: selo do documento no cabeçalho, **Unidade** como seleção (Embalagem/Requalificadora), e uma tabela **Itens Devolvidos** (Código do Produto · Descrição · Quantidade · Un.) — ao digitar o código, a **descrição é preenchida da base de produtos**. Mantém tipo (Fornecedor/Cliente), dados do cliente, ocorrência de origem, dados fiscais/transporte, tratativa e o bloco de rastreabilidade (emitido/aprovado). Funções: `devAddItem`, `devPreencherItem`, `devItensReset`. |
| **PCP — Programação das Rotativas** | Painel de sequenciamento por máquina impressora rotativa (**Gearless** e **Millennium**, abas), do modelo oficial de Programação. Colunas: OP/Obs · Entrega · Larg. · Cliente · SKU · Descrição · Qt. Prog. · Qt. Realiz. · **% realizado** · Início · **Dias** · **Término**. O sistema calcula sozinho os **dias** (pela capacidade kg/dia da máquina, editável), o **término** e encadeia o início da próxima OP no dia seguinte; **% e KPIs** (kg programados/realizados/atingido) ao vivo. Linhas editáveis inline, reordenáveis, com **Puxar da Fila** e **impressão**. Persistência por máquina. Funções: `renderProgramacao`, `progRecalc`, `progImportarFila`. |
| **PCP — Apontamentos de Produção** | Registro de apontamentos das rotativas por intervalo, do modelo oficial: dia/mês/ano · turno (1º/2º) · máquina · Operador 1/2 · **código do apontamento** (os **27 códigos oficiais**: 99-Produção, 4-Ajuste de Máquina, R-Refeição, 24-Setup, 18-Acerto de Cor, 21-Problemas com MP, 25-Problemas com clichê…) · Nº OP · SKU · Produção mt · kg · Aparas lisa/impressa · hora início/final (**hora total calculada**) · OBS. Lista com filtro e **KPIs** (produção mt/kg, aparas). Funções: `apoAdd`, `renderApontamentos`, `apoHoraTotal`. |
| **Fila & Calendário integrado (mesmo quadro nos 3 portais)** | A **Programação das rotativas é a fila de produção** e aparece como **um único quadro compartilhado** em **Qualidade** (Produção — Fila e Calendário), **PCP** (Fila de Produção) e **Produção** (Fila a Produzir) — mesmos dados, mesmo visual, portais diferentes. O board tem a **sequência em cartões** (OP, máquina, cliente, kg, datas início→término e % com barra de progresso, cor por máquina/status) e uma **linha do tempo (Gantt)** por máquina. Reforça o ciclo: a Qualidade gera as fichas e envia ao setor seguinte, o PCP imprime e sequencia — integrado, nada se perde. Funções: `filaBoardHTML`, `renderFilaBoard`, `filaGanttSVG`, `pcpProgAll`. |
| **PCP — Gestão à Vista (dashboard)** | Painel visual do PCP que vai **além das planilhas**, com gráficos **SVG nativos (offline)**: faixa de indicadores (OPs, atingido kg, **disponibilidade** = produção/(produção+paradas), horas paradas, aparas), **ocupação das máquinas** (Gantt da programação), **Realizado × Programado** por máquina (barras), **Paradas por motivo** (Pareto das horas apontadas por código, exclui produção) e **Produção diária** (metros apontados por dia). Tudo alimentado pela base real. Funções: `renderPcpPainel`, `gvRealVsProgSVG`, `gvParadasSVG`, `gvDiariaSVG`. |
| **PCP — Importar arquivos + base de teste** | Botão **Importar arquivo** na Programação e nos Apontamentos: **cole as células do Excel (Ctrl+C→Ctrl+V)** ou selecione um **.csv** — o sistema detecta o delimitador (tab/;/,), reconhece as colunas pelo **cabeçalho** (ou usa a ordem da planilha original), converte datas (dd/mm/aaaa, aaaa-mm-dd ou serial do Excel) e carrega para a máquina/coleção certa (adicionar ou substituir). O sistema já vem **alimentado como base de teste** com os dados reais das planilhas enviadas (Programação Gearless/Millennium e ~55 Apontamentos). O painel PCP ganhou **faixas de indicadores** (OPs, kg programados/atingido, em atraso; apontamentos, produção kg, aparas; kits gerados). Funções: `pcpImportProcessar`, `pcpParseLinhas`, `pcpMapear`; seed em `pcp-seed`. |
| **PCP — Kit da Ordem de Produção** | Gera o **kit de documentos que acompanha a OP** no chão de fábrica, do modelo kit_op: **Capa (Principal)**, **Solicitação de Retirada de MP** (bobinas + acessórios zip/fita/forros), **Controle de Paradas** (com a lista dos 27 códigos) e **Fechamento da OP** (produção, tintas/MP/insumos, movimentação de estoque). Preencha o cabeçalho (OP, SKU→descrição, cliente, máquina, largura, qtd), escolha quais folhas incluir e **imprima o kit** completo. Kits gerados ficam registrados (recarregar/remover). Funções: `kitImprimir`, `kitFolhaHTML`, `renderKitOps`. |
| **Placas de Identificação (RGQ-130)** | Novo módulo (Operação → Placas de Identificação) para **emitir e imprimir as placas de status/segregação** de materiais no chão de fábrica, do modelo oficial RGQ-130: 7 tipos — **Segregado · Aguardando Inspeção · Reposição · Não Utilize · Material Fosco · Apara · Lote de Recipiente**, cada um com cor própria. Preencha os dados de rastreio (produto/código, cliente, lote/NF, quantidade, data, responsável, nº RNC, motivo), **pré-visualize** a placa em folha inteira e imprima. Funções: `renderPlacasView`, `selecionarPlaca`, `placaDocHTML`, `imprimirPlaca`. |
