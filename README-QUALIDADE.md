...
| **Impressão fiel ao layout** | As Instruções de Trabalho saem na **mesma ordem do preenchimento na tela**, com os **subtítulos de grupo** (MP e bobina base, configuração de clichê e máquina, receita, estações/tintas, auto-controle…) e a **cor de cada etapa** (Clichê dourado, Impressão azul, Laminação roxo, Rebobinadeira ciano, Corte teal, Expedição vinho) no cabeçalho, títulos e grupos — controle visual imediato de qual documento é de qual setor. |
