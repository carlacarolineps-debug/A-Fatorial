...
| **Ficha Única (teste)** | **Um documento com todas as etapas**: marque no Fluxo de Processos quais etapas o produto passa (Clichê, Impressão, Laminação, Rebobinadeira, Corte & Solda, Expedição) e **só as seções marcadas se abrem** para preencher. Cada seção mostra progresso (`preenchidos/total`), há um **resumo vivo** (etapas, % preenchido, fluxo na ordem do POP) e **nenhum campo se perde**: tudo é serializado na ficha (`dados`) e restaurado ao reabrir. Ao digitar um produto que já tem Ficha Única, o sistema **oferece carregar os parâmetros da última versão** para só ajustar o que mudou. Ao salvar, a ficha entra no fluxo com setor = 1ª etapa e próximo setor = 2ª etapa. Funções: `salvarFichaUnica`, `fuCarregarFicha`, `fuChecarAnterior`, `fuToggleEtapa`, `fuAtualizarResumo`. |
