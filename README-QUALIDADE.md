...
| **Parâmetros em mini-tabelas agrupadas** | Na Impressão, a zona de Temperatura/Tensões/Pressões usa **mini-tabelas** como na ficha original: um cabeçalho escuro sobre pares de sub-colunas sobre os valores (CONTROLE DE TEMPERATURA °C → Entre Cores · Final · Calandra; REBOBINADOR — TENSÕES → Tensão Rebobinador · Tensão do Filme · Balancim; PRESSÕES → Contra Pressão · Velocidade · Contra Pressão Tambor · Frenagem · Pressão Compactador). Mesma apresentação no **preenchimento e na impressão**. |
