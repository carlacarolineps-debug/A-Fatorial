...
Run the sync script:
```bash
node .claude/skills/design-brand/scripts/sync-brand-to-tokens.cjs
```
...
node .claude/skills/design-brand/scripts/inject-brand-context.cjs --json | head -30
...
- `design-brand` - Brand context extraction and sync
