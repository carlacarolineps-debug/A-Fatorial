...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `design-ui-ux-pro-max`, `design-brand`, `design-system`, and `chrome-devtools` skills.
...
3. **Randomly invoke ONE of:** `/ck:design-ui-ux-pro-max` OR `/ck:frontend-design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
